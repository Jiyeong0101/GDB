...
def render_active_character_detail():
    ...
    try:
        detail_res = get_active_character(st.session_state.user_id, st.session_state.cookies)
        ...
        if skills:
            ...
            skill_df = skill_df[["skill_id", "skill_name", "skill_level", "mp_cost", "cooldown_sec", "description"]]
            skill_df.columns = ["스킬ID", "스킬명", "스킬 레벨", "MP 소모", "쿨타임", "설명"]
        ...
        if inventory:
            ...
            inventory_df = inventory_df[[
                "inventory_type", "item_name", "item_type", "item_sub_type", "quantity", "capacity"
            ]]
            inventory_df.columns = ["가방 종류", "아이템명", "아이템 타입", "세부 타입", "수량", "가방 용량"]
...
def render_character_list():
    ...
    try:
        char_res = get_characters(st.session_state.user_id, st.session_state.cookies)
    except Exception as e:
    ...
    st.subheader("🎯 캐릭터 선택")
    char_options = {f"{c['character_name']} (Lv.{c['level']})": c["actor_id"] for c in characters}
    ...
    with col1:
        if st.button("대표 캐릭터로 활성화"):
            try:
                res = activate_character(st.session_state.user_id, selected_id, st.session_state.cookies)
    ...
    with col2:
        ...
        if st.button("캐릭터 삭제", disabled=not confirm_char_delete):
            try:
                delete_res = delete_character(st.session_state.user_id, selected_id, st.session_state.cookies)
                if delete_res.status_code == 200:
            ...
            try:
                ...
                if create_res.status_code in [200, 201]:
                    st.success(
                        ...
                        "기본 스탯/스킬/인벤토리와 첫 전투 훈련 퀘스트가 지급됩니다."
                    )
                    st.rerun()
                else:
